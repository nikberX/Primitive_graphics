...
class PreLoader:
    ...
    def generateStep(self,cameraPos):
        ...
        self.M, self.s = CKM_to_CKH(self.M, [cameraPos[0],cameraPos[1],cameraPos[2]])
        self.M = parallel_proj(self.M, self.s)
        self.M = CKK_to_CKEi(self.M, 10, self.w / 2, self.h / 2, 200, 200)
...
def main():
    camera = Camera([0,0,-5])
    preloader = PreLoader(SCR_W,SCR_H,func,50,200)
